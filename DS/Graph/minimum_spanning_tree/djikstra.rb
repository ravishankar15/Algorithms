...
#       b. For every adjacent vertex v of u check if v is in heap. If v is in heap and the distance value of v is more than
#           the weight of u-v + distance value of u than update the value of v as sum of weight of u-v and distance value of u
class MinHeap
...
class Dijikstra
    ...
    def initialize(graph)
        @graph = graph
        @distance = Array.new(graph.no_vertices, 10**9);
    end
    ...
    def shortest_paths
        ...
        while (minheap.heap_size > 0)
            ...
            @graph.adj_list[min_node.vertex].each do |graph_node|
                if (minheap.is_node_present?(graph_node.dest) &&
                        @distance[min_node.vertex] != 10**9 &&
                        (@distance[min_node.vertex] + graph_node.weight) < @distance[graph_node.dest])

                    @distance[graph_node.dest] = @distance[min_node.vertex] + graph_node.weight
                    minheap.decrese_key(graph_node.dest, @distance[graph_node.dest])
                end
            end
        end

        (0).upto(@graph.no_vertices-1) do |vertex|
            puts "#{vertex} - #{@distance[vertex]}"
        end
    end
end
...
dijikstra = Dijikstra.new(graph)
dijikstra.shortest_paths